...
export default class whatsAppService {
  ...
  static async sendTemplateMessage(req: Request, res: Response) {
    ...
    try {
      ...
      const { leadPhoneNumber, templateName, parameterValues } = req.body;
      ...
      const responses = await Promise.all(
        payload.map(async (singlePayload: any) => {
          try {
            ...
            await handleTemplateMessageFlow(singlePayload.to, process.env.SalesAgent_Id, messageDetails);
  ...
  // export const createSalesAgent = tryCatchAsync(async (req: Request, res: Response) => {
  //   const companyId = req.companyId
  //   const salesAgentData = req.body
  //   const newSalesAgent = await SalesAgentService.createSalesAgent(salesAgentData, companyId)
  //   successResponse(req, res, 'Sales agent created successfully', newSalesAgent)
  // })
  ...
  static async createSalesAgent(req: Request, res: Response) {
    ...
    return successResponseWithData(res, "Sales Agent Created Successfully", newSalesAgent);
  }
  ...
  static getConversationBilling(req:Request,res:Response){

  }

  static getConversationHistory(req:Request,res:Response){
  ...
  static async salesAgentChats(req:Request,res:Response){
    ...
    try{
      ...
      // id:1, leadId:uuid, chat: Array(6)

      // const chats: any[] = await salesAgentChats.map((chat:any, index:number) => ({
      //   chat: salesAgentChats
      // }));
